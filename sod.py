...
USAGE ./sod.py -n 10000 --read-len 250 --sub-rate 0.1 --ins-rate 0.05 --del-rate 0.05 tests/hxb2.fasta > outfile.fa
HELP sod.py -h
...
def mutate(read_len, sub_rate, ins_rate, del_rate, indel_ext_rate):
    '''
    ...
    while read_i < read_len:
        ...
        if insertion:
            if read_i + insertion + 1 > read_len:
                insertion -= 1 + insertion + read_i - read_len
            read_i += insertion
        if deletion:
            if read_i + deletion + 1 > read_len:
                deletion -= 1 + deletion + read_i - read_len
            temp_i += deletion
...
def simulate_read(ref_f, ref_rc, ref_len, mut_len, ids_masks, i):
    ...
    return SeqRecord(Seq(read, DNAAlphabet()), id=read_header, description='')


def simulate_reads(path_to_ref, n_reads=1, read_len=150, sub_rate=0.0, ins_rate=0.0,
                   del_rate=0.0, indel_ext_rate=0.25, fastq=False, uppercase=False,):
    '''
    ...
    for i in range(n_reads):
        ids_masks, mut_len = mutate(read_len, sub_rate, ins_rate, del_rate, indel_ext_rate)
...
def main(path_to_ref, n_reads=1, read_len=150, sub_rate=0.0, ins_rate=0.0, del_rate=0.0,
         indel_ext_rate=0.25, fastq=False, uppercase=False):
    '''
    Command line interface
    '''
    records = simulate_reads(path_to_ref, n_reads, read_len, sub_rate,
